 ...
 * @author 谢森
 */
public class ReadLogFile2KafkaWithKey {
    public static void main(String[] args) {
        String topic = "ods_xiesen";
//        String filePath = "D:\\tmp\\kcbp\\filebeat";
        String filePath = "D:\\tmp\\kcbp\\kcbp.log";
...
//                System.out.println("第[" + lineCount + "]行数据:" + strLine);
                final Map map = JSONObject.parseObject(strLine, Map.class);

                final Map logMap = MapUtils.getMap(map, "log");
                final String message = MapUtils.getString(map, "message");
                final Map fileMap = MapUtils.getMap(logMap, "file");
                final Long offset = MapUtils.getLong(logMap, "offset");
                final String path = MapUtils.getString(fileMap, "path");
                System.out.println(path + " " + message + " " + offset);

                if (null != strLine) {
//                    send(producer, topic, strLine, path);
                    lineCount++;
                }
            }
...
//                log.info("第[" + lineCount + "]行数据:[" + strLine + "]");
     ...
     * @param <T>
     */
    private static <T> void send(KafkaProducer<String, T> producer, String topic, T message, String key) {
