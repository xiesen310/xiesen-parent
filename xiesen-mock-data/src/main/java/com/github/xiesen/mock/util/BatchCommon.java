...
public class BatchCommon<T> {
    private static final Logger logger = LoggerFactory.getLogger(BatchCommon.class);
     ...
     * 时间触发任务
     */
    public Thread timeJob;

    /**
     * 函数式编程,将方法当成参数,这个函数可以不用返回值,Function必须有返回值
     */
    public Consumer<List<T>> function;
    /**
     * 时间格式化
     */
    ...
    /**
     * 两次休眠之间是否有达到条数触发,修改变量有锁,一定误差计时的误差范围是[0,最大等待时间]
     */
    ...
    /**
     * 达到一定时间触发任务
     * 达到时间执行与达到数量执行两个方法需要加锁,即 waitMilliSecond 与 add
     */
