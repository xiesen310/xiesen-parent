...
/**
 * @author xiese
 * @Description An immutable polynomial in the finite field GF(2^k)
...
public class Polynomial implements Arithmetic<Polynomial>, Comparable<Polynomial> {
    ...
    public static Polynomial createIrreducible(int degree) {
        while (true) {
            byte[] bytes = new byte[]{127, -69, 28, 21, 30, -77, -67};
            Polynomial p = createFromBytes(bytes, degree);
            /**
             * 源码中自动生成的，这里我们为了确认对象的唯一性，将对象进行固化
